...
public class PackingRecommendationService {
    ...
    private String callClaudeAPI(String userPrompt, String conversationId) {
        try {
            ...
            log.info("패킹 추천 AI 호출 - Provider: {}", aiProviderService.getPrimaryProvider());
            ...
            if (!aiResponse.isSuccess()) {
                log.warn("패킹 추천 AI 응답 실패 - Provider: {}, 원인: {}",
                        aiResponse.getProvider(), aiResponse.getErrorMessage());
                throw new RuntimeException("AI 응답 실패: " + aiResponse.getErrorMessage());
            }

            log.info("패킹 추천 AI 응답 성공 - Provider: {}, 토큰: {}",
    ...
    /**
     * AI API 호출 (대화 기록 포함)
     * - 대화 기록의 마지막 메시지를 기반으로 AI 호출
     * - 대화 컨텍스트를 유지하기 위해 이전 대화 내용을 프롬프트에 포함
     */
    private String callClaudeAPIWithHistory(String userMessage, List<Map<String, String>> history) {
        try {
            ...
            for (int i = 0; i < history.size() - 1; i++) {
                ...
                contextPrompt.append(role.equals("user") ? "사용자: " : "AI: ");
                contextPrompt.append(content).append("\n\n");
            }
            contextPrompt.append("현재 사용자 메시지: ").append(userMessage);

            log.info("패킹 대화 AI 호출 - Provider: {}, 대화 기록 수: {}",
                    aiProviderService.getPrimaryProvider(), history.size());
            ...
            if (!aiResponse.isSuccess()) {
                log.warn("패킹 대화 AI 응답 실패 - Provider: {}, 원인: {}",
                        aiResponse.getProvider(), aiResponse.getErrorMessage());
                throw new RuntimeException("AI 응답 실패: " + aiResponse.getErrorMessage());
            }

            log.info("패킹 대화 AI 응답 성공 - Provider: {}, 토큰: {}",
                    aiResponse.getProvider(), aiResponse.getTotalTokens());
