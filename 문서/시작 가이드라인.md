# 🚀 Wanderlust 로컬 개발 가이드

AI 여행 동행 매칭 플랫폼 로컬 개발 환경 설정 및 실행 가이드

---

## 📋 시스템 요구사항

- **Java**: 17 이상 (Oracle JDK 또는 OpenJDK)
- **Maven**: 3.6 이상
- **MariaDB**: 10.5 이상 (또는 MySQL 8.0+)
- **운영체제**: Windows 10/11, macOS, Linux

---

## 🔧 초기 설정

### 1. Java 설치 확인

```bash
java -version
# 출력: openjdk version "17.0.x" 또는 java version "17.0.x"
```

### 2. Maven 설치 확인

```bash
mvn -version
# 출력: Apache Maven 3.x.x
```

### 3. MariaDB 설치 및 데이터베이스 생성

```bash
# MariaDB 접속
mysql -u root -p

# 데이터베이스 생성
CREATE DATABASE IF NOT EXISTS tt_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

# 사용자 권한 확인 (필요시)
GRANT ALL PRIVILEGES ON tt_db.* TO 'root'@'localhost';
FLUSH PRIVILEGES;

# 종료
EXIT;
```

### 4. 환경 변수 설정

`application.properties` 파일의 기본값을 사용하거나, 환경 변수로 오버라이드 가능:

```bash
# 데이터베이스 설정
export SPRING_DATASOURCE_URL=jdbc:mariadb://localhost:3306/tt_db
export SPRING_DATASOURCE_USERNAME=root
export SPRING_DATASOURCE_PASSWORD=your_password

# 파일 업로드 경로 (선택)
export UPLOAD_PATH=uploads/
```

### 5. ⚠️ Claude API 키 설정 (필수)

AI 채팅 기능을 사용하려면 Anthropic Claude API 키가 필요합니다.

1. [Anthropic Console](https://console.anthropic.com/)에서 API 키 발급
2. 환경 변수로 설정:

```bash
# Windows
set CLAUDE_API_KEY=sk-ant-api03-your-key-here

# Mac/Linux
export CLAUDE_API_KEY=sk-ant-api03-your-key-here
```

> **Note**: API 키 없이도 기본 기능은 사용 가능하지만, AI 채팅 기능은 작동하지 않습니다.

---

## ⚡ 빠른 시작

### 방법 1: Spring Boot 실행 (권장)

#### 개발 환경 (기본)
```bash
# 프로젝트 디렉토리로 이동
cd /path/to/wanderlust_new

# 외부 DB 연결 (211.178.142.239)
mvn spring-boot:run
```

#### 테스트 환경 (내부 네트워크)
```bash
# 서버 내부 IP 연결 (100.75.197.18)
mvn spring-boot:run -Dspring-boot.run.profiles=test
```

**접속**: http://localhost:8080

### 방법 2: Jetty 서버 실행

```bash
mvn jetty:run
```

**접속**: http://localhost:8111

### 방법 3: WAR 파일 빌드 및 배포

```bash
# WAR 파일 생성
mvn clean package

# 생성된 파일: target/tour-project.war
# Tomcat 등의 서블릿 컨테이너에 배포 가능
```

---

## 🔄 프로파일 가이드

### 사용 가능한 프로파일

| 프로파일 | 데이터베이스 | 용도 | 실행 방법 |
|----------|-------------|------|----------|
| **dev** (기본) | 211.178.142.239:3306 | 외부 원격 DB | `mvn spring-boot:run` |
| **test** | 100.75.197.18:3306 | 내부 네트워크 직접 연결 | `mvn spring-boot:run -Dspring-boot.run.profiles=test` |
| **prod** | mariadb-11.4.5:3306 | Docker 운영 환경 | `docker-compose up -d` |

### 프로파일 전환

```bash
# 개발 환경 (기본값)
mvn spring-boot:run

# 테스트 환경
mvn spring-boot:run -Dspring-boot.run.profiles=test

# 환경 변수로 설정
export SPRING_PROFILES_ACTIVE=test
mvn spring-boot:run
```

---

## 🏗️ 프로젝트 구조

```
wanderlust_new/
├── src/main/java/com/tour/project/
│   ├── Application.java           # Spring Boot 메인 클래스
│   ├── client/                    # 외부 API 클라이언트 (Claude API)
│   ├── config/                    # Spring 설정 (Security, WebSocket, DB)
│   ├── controller/                # HTTP 요청 처리
│   ├── dao/                       # MyBatis 매퍼 인터페이스
│   ├── dto/                       # 데이터 전송 객체
│   ├── service/                   # 비즈니스 로직
│   ├── util/                      # 유틸리티 클래스
│   └── vo/                        # Value Object (도메인 엔티티)
├── src/main/resources/
│   ├── application.properties     # 애플리케이션 설정
│   └── com/tour/project/mapper/   # MyBatis XML 매퍼
├── src/main/webapp/
│   └── WEB-INF/views/             # JSP 뷰 파일
└── pom.xml                        # Maven 의존성 관리
```

---

## 📊 주요 기술 스택

- **Framework**: Spring Boot 2.7.15
- **View**: JSP + JSTL
- **Database**: MariaDB (MyBatis ORM)
- **Security**: Spring Security
- **WebSocket**: STOMP
- **AI**: Claude API (Anthropic)
- **Build**: Maven

---

## 🔍 개발 모드 최적화

### Hot Reload 활성화

```bash
# 코드 변경 시 자동 재시작
mvn spring-boot:run -Dspring-boot.run.fork=false

# Devtools 활용 (pom.xml에 추가 필요)
<dependency>
    <groupId>org.springframework.boot</groupId>
    <artifactId>spring-boot-devtools</artifactId>
    <optional>true</optional>
</dependency>
```

### JVM 메모리 설정

```bash
mvn spring-boot:run -Dspring-boot.run.jvmArguments="-Xmx1024m -Xms512m"
```

### 특정 프로파일 실행

```bash
# 개발 환경 프로파일
mvn spring-boot:run -Dspring-boot.run.profiles=dev
```

---

## 🛠️ 문제 해결

### ❌ Port 8080 already in use

```bash
# Mac/Linux
lsof -i :8080
kill -9 [PID]

# Windows
netstat -ano | findstr :8080
taskkill /F /PID [PID]
```

### ❌ Database Connection Failed

```bash
# MariaDB 실행 확인
mysql -u root -p -e "SELECT 'DB OK' as status;"

# 데이터베이스 존재 확인
mysql -u root -p -e "SHOW DATABASES LIKE 'tt_db';"
```

### ❌ Java Version Mismatch

```bash
# Java 버전 확인
java -version

# JAVA_HOME 설정 (필요시)
export JAVA_HOME=/path/to/java-17
```

### ❌ Maven Build Failed

```bash
# 캐시 정리 후 재빌드
mvn clean install -U

# 테스트 생략하고 빌드
mvn clean package -DskipTests
```

---

## 🎯 주요 엔드포인트

### 웹 페이지

- **메인**: http://localhost:8080
- **게시판**: http://localhost:8080/board/list
- **여행 계획**: http://localhost:8080/travel/list
- **AI 채팅**: http://localhost:8080/ai/chat
- **관리자**: http://localhost:8080/admin

### API 엔드포인트

- **Health Check**: http://localhost:8080/actuator/health
- **WebSocket**: ws://localhost:8080/ws/chat

---

## 🐳 Docker 배포

### Docker Compose 실행

```bash
# 컨테이너 시작
docker-compose up -d

# 로그 확인
docker-compose logs -f wanderlust-app

# 컨테이너 중지
docker-compose down
```

### 프로덕션 배포

GitLab CI/CD 파이프라인을 통한 자동 배포:

```bash
# main 브랜치에 push하면 자동 배포
git push origin main

# 배포 확인
curl https://wanderlust.ibetter.kr/actuator/health
```

자세한 배포 가이드는 다음 문서 참조:
- [서버 초기 설정](../Obsidian/지식노트/1_Projects/2025-10_wanderlust/wanderlust_new/서버_초기_설정.md)
- [GitLab CI/CD 설정](../Obsidian/지식노트/1_Projects/2025-10_wanderlust/wanderlust_new/GitLab_CI_CD_설정.md)
- [배포 프로세스](../Obsidian/지식노트/1_Projects/2025-10_wanderlust/wanderlust_new/배포_프로세스.md)

---

## 📝 개발 워크플로우

### 1. 기능 브랜치 생성

```bash
git checkout -b feature/기능명
```

### 2. 개발 및 테스트

```bash
# 서버 실행
mvn spring-boot:run

# 테스트 실행
mvn test
```

### 3. 커밋 및 푸시

```bash
# 한글 커밋 메시지 사용
git commit -m "feat: 새로운 기능 추가"
git push origin feature/기능명
```

### 4. Pull Request 생성

GitLab에서 Pull Request 생성 후 리뷰 요청

---

## 🔒 보안 설정

### Claude API 키 설정

`.env` 파일 또는 환경 변수로 API 키 설정 (절대 Git에 커밋하지 말것):

```bash
export CLAUDE_API_KEY=sk-ant-api03-...
```

### Spring Security

기본 보안 설정은 `SecurityConfig.java`에서 관리

---

## 📚 추가 문서

- **프로젝트 개요**: `CLAUDE.md`
- **API 문서**: `docs/API.md` (생성 예정)
- **데이터베이스 스키마**: `src/main/resources/schema.sql`

---

**최종 업데이트**: 2025-10-29
**문의**: 프로젝트 관리자
